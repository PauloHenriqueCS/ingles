...
// Shared, deterministic cache for voice preview samples (identical for every
// user). Reuses the existing private audio bucket; a dedicated prefix keeps the
// namespace clear. Path per (previewVoice, pace) → generated at most once each.
const PREVIEW_CACHE_BUCKET = 'listening-audio';
const PREVIEW_CACHE_PREFIX = 'voice-previews/gpt-4o-mini-tts';
...
async function handlePreview(req: any, res: any) {
  ...
  // Voice previews are IDENTICAL for every user (fixed phrase per voice+pace),
  // so they are cached once in the shared audio bucket and reused — a ▶ click
  // must NOT trigger a new paid TTS call when a cached sample already exists.
  // Bounded set (voices × paces), generated at most once each. Best-effort:
  // any storage hiccup just falls through to (re)generation, never fails.
  const cachePath = `${PREVIEW_CACHE_PREFIX}/${previewVoice}_${pace}.mp3`;
  // Defensive: getSharedServiceClient()/download are fully inside try so any
  // storage/config issue silently degrades to (re)generation, never a crash.
  let cachedBuf: Buffer | null = null;
  try {
    const { data: cached } = await getSharedServiceClient().storage.from(PREVIEW_CACHE_BUCKET).download(cachePath);
    if (cached) {
      const buf = Buffer.from(await cached.arrayBuffer());
      if (buf.length > 0) cachedBuf = buf;
    }
  } catch { /* cache miss / storage hiccup → generate below */ }
  if (cachedBuf) {
    safeLog('conversation/preview', 'cache_hit', 200);
  ...
  try {
    audioBuffer = await executeAiGatewayCall<ArrayBuffer>(
      {
        ...
        // gpt-4o-mini-tts (not the legacy tts-1) — tts-1 rejects the newer
        // expressive voices (coral, ash, ballad, sage, verse) with HTTP 400,
        // which was the root cause of "Não foi possível carregar a amostra."
        // gpt-4o-mini-tts supports all voices this app exposes.
      ...
      async () => {
        ...
        try {
          ttsRes = await fetch(TTS_URL, {
            ...
            body: JSON.stringify({ model: 'gpt-4o-mini-tts', voice: previewVoice, input, speed, response_format: 'mp3' }),
            signal: ctrl.signal,
          });
        } catch (err) {
    ...
    // Any other error (e.g. a GatewayError from the enforce pipeline) must not
    // escape as an unhandled 500 crash — return a clean JSON 5xx.
    safeLog('conversation/preview', 'unexpected_error', 500, { name: err instanceof Error ? err.name : typeof err });
    return res.status(500).json({ code: 'PREVIEW_FAILED', message: 'Não foi possível gerar a amostra.' });
  }

  // Populate the shared cache (best-effort) so subsequent previews of this
  // voice+pace serve from storage with no provider call. upsert keeps the
  // deterministic path stable and never creates duplicates.
  try {
    await getSharedServiceClient().storage.from(PREVIEW_CACHE_BUCKET).upload(cachePath, Buffer.from(audioBuffer), { contentType: 'audio/mpeg', upsert: true });
  } catch { /* non-fatal — the freshly generated bytes are still returned below */ }
...
async function handleSession(req: any, res: any) {
  ...
  const identityCols = {
    ...
    // FROZEN with the row so the session's language mode stays consistent for
    // its whole lifetime (mirrors session_mode). Purely informational for
    // accounting/credit — those never read it. Inserted atomically in the
    // commercial path and re-applied by the freeze UPDATE for the trial path.
    conversation_language_mode: languageMode,
