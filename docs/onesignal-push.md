...
- **Large notification icon** — `android/app/src/main/res/drawable-xxxhdpi/ic_onesignal_large_icon_default.png`, 256×256 PNG, the **full-colour** Orodim logo (from `public/brand/lemon-icon.png`). OneSignal auto-resolves `ic_onesignal_large_icon_default`. Shown on the right of expanded notifications and on the collapsed card on some launchers; the small icon is what appears in the status bar and, tinted, in the notification header.
