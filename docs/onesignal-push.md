...
- **Notification small icon** — `android/app/src/main/res/drawable-*/ic_stat_onesignal_default.png`, one per density at the Material/OneSignal-mandated pixel sizes: **mdpi 24, hdpi 36, xhdpi 48, xxhdpi 72, xxxhdpi 96**. Monochrome silhouette of the Orodim lettermark defined purely by the **alpha channel** (RGB is solid white; Android tints the alpha mask), transparent background, no color/gradient/shadow. OneSignal auto-resolves a drawable named `ic_stat_onesignal_default`.
  - Derived from the official brand silhouette (`ic_launcher_monochrome`, the themed-icon layer) so the mark stays consistent with the launcher — but **trimmed of the adaptive-icon safe-zone padding and resized to fill ~84% of each canvas** (8% keyline margin). The earlier assets were a verbatim copy of the 108-px monochrome layer (wrong size + ~45% padding), so after Android downscaled the oversized canvas to 24 dp the glyph shrank to a tiny centred blob that Samsung One UI rendered as effectively blank. Regenerate with the `scratchpad/gen_small.js` helper if the brand silhouette changes.
- **Large notification icon** — **intentionally not set.** OneSignal auto-resolves `ic_onesignal_large_icon_default` if present, but Samsung One UI already renders the app's (colour) launcher icon as the notification avatar on the left. Shipping a large icon that is the *same* Orodim logo made the notification show the brand **twice** (avatar on the left + large icon on the right) — redundant clutter. The large-icon slot is for a *distinct* image (sender photo, content thumbnail); for brand-only pushes the status-bar small icon + the launcher avatar are enough. Add `ic_onesignal_large_icon_default.png` (256×256, `drawable-xxxhdpi/`) only if a push needs a complementary right-hand image.
- **Accent color** — `onesignal_notification_accent_color` string in `res/values/strings.xml` = `FF009999` (AARRGGBB), the dominant brand teal from the Orodim logo. Tints the small icon and app name in the notification header. Overridable per-push, but not required for correct default branding.
